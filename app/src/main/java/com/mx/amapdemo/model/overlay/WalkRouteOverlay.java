package com.mx.amapdemo.model.overlay;
...
import com.mx.amapdemo.utils.OverLayUtil;

import java.util.List;

/**
 * 步行路线图层类。
 * 在高德地图API里，如果要显示步行路线规划，可以用此类来创建步行路线图层。
...
public class WalkRouteOverlay extends RouteOverlay {
	...
	public WalkRouteOverlay(Context context, AMap amap, WalkPath path,
		...
		this.mAMap = amap;
		this.walkPath = path;
		startPoint = OverLayUtil.convertToLatLng(start);
		endPoint = OverLayUtil.convertToLatLng(end);
	}
	/**
	 * 添加步行路线到地图中。
	 * @since V2.1.0
    ...
    public void addToMap() {
        ...
        try {
            ...
            for (int i = 0; i < walkPaths.size(); i++) {
                ...
                LatLng latLng = OverLayUtil.convertToLatLng(walkStep
                        .getPolyline().get(0));
    ...
    private void addWalkPolyLine(LatLonPoint pointFrom, LatLonPoint pointTo) {
        addWalkPolyLine(OverLayUtil.convertToLatLng(pointFrom), OverLayUtil.convertToLatLng(pointTo));
    }
    ...
    private void addWalkPolyLines(WalkStep walkStep) {
        mPolylineOptions.addAll(OverLayUtil.convertArrList(walkStep.getPolyline()));
    }
